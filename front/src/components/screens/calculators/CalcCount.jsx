...
const CalcCount = ({ isOpen, onClose }) => {
  ...
  const calculate = () => {
    ...
    if (!isNaN(absError) && !isNaN(measurementX)) {
      ...
      const calculatedUC = Math.sqrt(
        Math.pow(parseFloat(calculatedUBDelta), 2)
      ).toFixed(2);
      setUC(calculatedUC);
      ...
      const formattedResult = `(${measurementX.toFixed(
        2
      )} ± ${calculatedU}) ; k = ${k}; P = ${P.toFixed(2)}.`;
      ...
      <form onSubmit={onSubmit} className="space-y-12">
        <div className="px-6">
          <label
            ...
            className="text-md border-gray-900/10 font-semibold"
          >
        ...
        <div className="border-b border-gray-900/10 pb-12">
          ...
          <div className="mt-6 grid grid-cols-2 gap-6">
            {[
              "Расчёт стандартных неопределённостей (Ub∆=)",
              "Расчёт суммарной неопределённости (Uc)",
              "Вычисление расширенной неопределённости (U)",
              "Представление результатов оценивания неопределённости (X)",
            ].map((label, index) => (
              <div key={index} className="flex items-center px-6">
                <label
                  htmlFor={`calculation${index + 1}`}
                  className="text-sm text-gray-900 w-1/2"
                >
                  {label}:
                </label>
                <input
